...
class ServicesController extends Controller
{
    ...
    // =========================================
    // SESSION-BASED BRANCH SERVICES (Admin's Own Branch)
    // =========================================

    // Get services for the logged-in admin's branch (using session)
    // API endpoint: GET /services/myBranchServices
    public function myBranchServices()
    {
        header('Content-Type: application/json');
        ...
        if (!$branchId) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'No branch assigned to this admin']);
            return;
        }
    ...
    // Create branch service for logged-in admin's branch
    // API endpoint: POST /services/myBranchService
    public function myBranchServiceStore()
    {
        header('Content-Type: application/json');
        ...
        $requiredFields = ['default_service_id'];
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                http_response_code(400);
                echo json_encode(array('success' => false, 'error' => "Missing required field: $field"));
    ...
    // Create new default service with branch override for logged-in admin's branch
    // API endpoint: POST /services/myBranchServiceWithNew
    public function myBranchServiceWithNew()
    {
        header('Content-Type: application/json');
    ...
    // Update branch service for logged-in admin's branch
    // API endpoint: PUT /services/myBranchService/{id}
    public function myBranchServiceUpdate($id)
    {
        header('Content-Type: application/json');
        ...
        // Verify the service belongs to this branch
        $service = $this->servicesModel->getBranchServiceById($id);
        if (!$service || $service['branch_id'] != $branchId) {
            http_response_code(403);
            echo json_encode(array('success' => false, 'error' => 'Service not found in your branch'));
            return;
        }
    ...
    // Delete branch service for logged-in admin's branch
    // API endpoint: DELETE /services/myBranchService/{id}
    public function myBranchServiceDestroy($id)
    {
        header('Content-Type: application/json');
    ...
    // Get categories for the logged-in admin's branch (using session)
    // API endpoint: GET /services/myBranchCategories
    public function myBranchCategories()
    {
        header('Content-Type: application/json');
    ...
    // Create branch category for logged-in admin's branch
    // API endpoint: POST /services/myBranchCategory
    public function myBranchCategoryStore()
    {
        header('Content-Type: application/json');
        ...
        $requiredFields = ['default_category_id'];
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                http_response_code(400);
                echo json_encode(array('success' => false, 'error' => "Missing required field: $field"));
        ...
        // Handle duplicate entry - try to update instead
        if (!$categoryId) {
            // Find existing record ID
            $existing = $this->servicesModel->getBranchCategoryByBranchAndDefault($branchId, $data['default_category_id']);
            if ($existing) {
                $updateData = array(
                    'display_name' => $data['display_name'] ?? null,
                    'description_override' => $data['description_override'] ?? null,
                ...
                $this->servicesModel->updateBranchCategoryOverride($existing['branch_category_override_id'], $updateData);
                http_response_code(200);
                echo json_encode(array(
                    'success' => true,
                    'message' => 'Branch category updated successfully',
                    'data' => array('branch_category_override_id' => $existing['branch_category_override_id'])
                ));
                return;
            }
        }
    ...
    // Update branch category for logged-in admin's branch
    // API endpoint: PUT /services/myBranchCategory/{id}
    public function myBranchCategoryUpdate($id)
    {
        header('Content-Type: application/json');
        ...
        // Verify the category belongs to this branch
        $category = $this->servicesModel->getBranchCategoryById($id);
        if (!$category || $category['branch_id'] != $branchId) {
            http_response_code(403);
            echo json_encode(array('success' => false, 'error' => 'Category not found in your branch'));
            return;
        }
    ...
    // Delete branch category for logged-in admin's branch
    // API endpoint: DELETE /services/myBranchCategory/{id}
    public function myBranchCategoryDestroy($id)
    {
        header('Content-Type: application/json');
